...
def kinectConnectionCheck() -> str:
    ...
    try:
        ...
        if res:
            return str(device.get_serialnum())  # 如果连接成功，返回序列号
        else:
...
class KinectDKThread(QThread):
    ...
    def run(self):
        ...
        if _run_flag:  # 只有相机连接成功才有必要进行下面的操作
            ...
            device_config.color_resolution = pykinect.K4A_COLOR_RESOLUTION_1536P # 彩色流分辨率为720p
            device_config.depth_mode = pykinect.K4A_DEPTH_MODE_WFOV_2X2BINNED  # 深度模式设置为去畸变窄视场  K4A_DEPTH_MODE_NFOV_UNBINNED
            ...
            while True:
                try:
                    ...
                    flag_ir = True
                    flag_capture = gv.getValue('baggageCaptureFlag')  # 检查点云采集标志
                    ...
                    if flag_capture:  # 如果需要采集点云
                        ...
                        cv2.imwrite(f'./data/baggage_depth{dt}.jpg', depth_image)
                    if flag_capture:  # 如果需要采集点云
                        flag_ir , ir_image = capture.get_ir_image() # 分离深度帧 capture.get_colored_depth_image()  capture.get_transformed_colored_depth_image()
                        dt = datetime.datetime.now().strftime('%Y%m%d%H%M%S')
                        cv2.imwrite(f'./data/baggage_ir{dt}.jpg', ir_image)
...
#####################################################################################################################################
                    ...
                    if flag_capture_2nd:  # 如果需要采集点云
                        ...
                        if gv.getValue('saveBaggageFlag_2nd'):  # 如果需要保存
                            ...
                            o3d.io.write_point_clozud(f'./data/2nd_baggage_latest.ply', pcd)
