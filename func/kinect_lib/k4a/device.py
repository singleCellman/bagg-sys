...
from ._k4atypes import K4A_WAIT_INFINITE
...
class Device:
	...
	def start(self, configuration, record=False, record_filepath="output.mkv"):
		...
		res = self.start_cameras(configuration)
		if not res:
			return False
		self.start_imu()
	...
	def start_cameras(self, device_config):
		...
		return _k4a.VERIFY(_k4a.k4a_device_start_cameras(self._handle, device_config.handle()), "Start K4A cameras failed!")
...
class Device1:
	...
	def start_cameras(self, device_config):
		...
		return _k4a.VERIFY(_k4a.k4a_device_start_cameras(self._handle, device_config.handle()),
