...
leaf_nodes        : A placement vector, [x1, y1, z1, x2, y2, z2]
                    ...
                    x2 = x1 + x, y2 = y1 + y, z2 = z1 + z
                    x, y, z are  sizes of the current item after an axis-aligned orientation 
                    (a little different from the original paper,
                    these two description have similar performance.).
...
def get_args():
    ...
    parser.add_argument('--no-cuda', default=True, help='Forbidden cuda')
    ...
    parser.add_argument('--evaluate', default=True, help='Evaluate only')
    parser.add_argument('--evaluation-episodes', type=int, default=1, metavar='N', help='Number of episodes evaluated')
    parser.add_argument('--load-model', default=True, help='Load the trained model')
    parser.add_argument('--model-path', type=str, help='The path to load model')
    parser.add_argument('--load-dataset', default=False,
    ...
    parser.add_argument('--sample-from-distribution', default=True,
    ...
    args.model_path = gv.getParam('stackModelPath')
    ...
    if gv.getParam('modelContinuous'):
...
def registration_envs():
    register(
        ...
        entry_point='stack.pct_envs.PctDiscrete0:PackingDiscrete',  # Expalined in envs/__init__old.py
    )
    register(
        id='PctContinuous-v0',
        entry_point='stack.pct_envs.PctContinuous0:PackingContinuous',
    )
