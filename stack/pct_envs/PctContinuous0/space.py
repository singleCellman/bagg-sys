            ...
            if len(self.bottom_edges) == 1:
                ...
                for e in self.bottom_edges:
                    if self.thisStack.centre[0] - e.area[0] > 1e-6 and e.area[2] - self.thisStack.centre[0] > 1e-6 \
                            and self.thisStack.centre[1] - e.area[1] > 1e-6 and e.area[3] - self.thisStack.centre[
                        1] > 1e-6:
                        direct_edge = e
                        break
    ...
    def calculated_impact_virtual(self, first=False):
        ...
        if len(self.bottom_edges) == 0:
            ...
            if len(self.bottom_edges) == 1:
                ...
                direct_edge = None
                for e in self.bottom_edges:
                    if self.thisVirtualStack.centre[0] - e.area[0] > 1e-6 and e.area[2] - self.thisVirtualStack.centre[
                        0] > 1e-6 \
                            and self.thisVirtualStack.centre[1] - e.area[1] > 1e-6 and e.area[3] - \
                            self.thisVirtualStack.centre[1] > 1e-6:
                        direct_edge = e
                        break
...
class Space(object):
    ...
    def drop_box(self, box_size, idx, flag, density, setting, **kwags):
        ...
        if sta_flag and self.box_idx < len(self.upLetter):  # 增加了对box序号的判断，防止溢出
    ...
    def interSectEMS3D(self, itemLocation):
        ...
        signal = (intersect[:, 0] + intersect[:, 3] > 0) * (intersect[:, 1] + intersect[:, 4] > 0) * (
                    intersect[:, 2] + intersect[:, 5] > 0)
