from stack.pct_envs.PctDiscrete0.space import Space
import numpy as np
import gym
from stack.pct_envs.PctDiscrete0.binCreator import RandomBoxCreator, LoadBoxCreator, BoxCreator
import torch
import random
import global_value as gv
from stack.pct_envs.PctDiscrete0.space import Box
...
class PackingDiscrete(gym.Env):
    def __init__(self,
                 ...
                 internal_node_holder=cfg.internal_node_holder, leaf_node_holder=cfg.leaf_node_holder, next_holder=1,
                 shuffle=False,
                 LNES='EMS',
                 **kwags):
    ...
    def reset(self):
        ...
        self.packed = []
        self.space.reset()
        stack_items = gv.getValue('stackItems')  # 加载垛型
        count = 0
        if stack_items is not None:  # 如果垛型为空则跳过
            for item in stack_items:
                # 将垛型实例化为Box实例
                _item = Box(item[3], item[4], item[5], item[0], item[1], item[2], 1)  # x尺寸，y尺寸，z尺寸，xyz坐标，密度（固定为1）
                self.space.boxes.append(_item)  # 记录垛型
                self.space.plain = self.space.update_height_graph(self.space.plain, _item)  # 更新高度矩阵
                self.space.height = max(self.space.height, 25)  # # 记录最高尺寸
                # 记录垛型向量，最后三位固定为101
                self.space.box_vec[self.space.box_idx] = np.array(
                    [item[0], item[1], item[2], item[3], item[4], item[5], 1, 0, 1])
                # 计算EMS
                self.space.GENEMS([item[0], item[1], item[2],
                                   item[0] + item[3], item[1] + item[4],
                                   item[2] + item[5]])
                # 记录已有垛型
                self.packed.append(
                    [item[0], item[1], item[2], item[3], item[4], item[5], count])
                count += 1
        self.box_creator.generate_box_size()  # 改动箱子生成顺序
    ...
    def step(self, action):
        ...
        self.box_creator.generate_box_size()  # 在每个step开始时生成一个箱子
        reward = box_ratio * 10
