...
import copy
from stack.pct_envs.PctDiscrete0.convex_hull import ConvexHull, point_in_polygen
from stack.pct_envs.PctDiscrete0.PctTools import AddNewEMSZ, maintainEventBottom, smallBox, extreme2D, corners2D
...
class Space(object):
    def __init__(self, width=cfg.spaceWidth, length=cfg.spaceLength, height=cfg.spaceHeight, size_minimum=0, holder=60):
    ...
    def drop_box(self, box_size, idx, flag, density, setting):
        ...
        if sta_flag:
            ...
            # 找出在规划位置前的所有垛型
            max_h_new = max_h
            if gv.getValue('stackItems') is not None:
                for items in gv.getValue('stackItems'):
                    if (items[0] < lx < items[0] + items[3]) \
                            or (items[0] < lx + x < items[0] + items[3]) \
                            and (items[1] + items[4] < ly):
                        max_h_new = max(max_h, items[5])  # 如果符合条件的垛型比规划位置还高，则以其高度更新规划位置的高，即实现抛放
            stack_mode = '正常' if max_h_new <= max_h else '抛放'
            gv.setValue('drawPlanPositionArray', np.array([[lx, ly, max_h_new]]))
            gv.setValue('drawPlanSizeArray', np.array([[x, y, z]]))
            gv.setValue('positionX', lx)
            gv.setValue('positionY', ly)
            gv.setValue('positionZ', max_h_new)
            gv.setValue('pickMode', '正常' if x < y else '横接')
            gv.setValue('stackMode', stack_mode)
            print('放置角点坐标：{a}, {b}, {c}；目标尺寸：{d}, {e}, {f}'.format(a=lx, b=ly, c=max_h, d=x, e=y, f=z))
            return True
        return False
